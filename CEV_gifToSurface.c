...
/*redraw method appiled to Texture*/
static void L_gifTextureRedraw(const uint8_t* pixels, const SDL_Rect* blitPos, SDL_Texture* dstTex);

/*overlay method applied to texture*/
static void L_gifTextureOverlay(const uint8_t* pixels, const SDL_Rect* blitPos, SDL_Texture* dstTex);

/*is coord x,y int rect ?*/
...
{//color struct to int*/
    ...
    return 0U | color.r<<24 | color.g<<16 | color.b<<8 | color.a;
}
