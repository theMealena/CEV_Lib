...
{//gif animation load from SDL_RWops
    ...
    for (int i=0; i<anim->status.imgNum; i++)
    {
        ...
        if(pixels != NULL)
        {
            ...
            {
                case 0:
                    GIFL_gifFillSurface(pixels, gif, i);
                break;

                case 1:
                    GIFL_gifFillSurfaceInterlace(pixels, gif, i);
                break;
        ...
        {
            fprintf(stderr,"Err at %s / %d : unable to allocate pixels : %s\n", __FUNCTION__, __LINE__, strerror(errno));
            goto err_3;
        }
